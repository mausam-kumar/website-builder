...
const Editor = () => {
    ...
    return <div className='flex-1 border shadow-lg p-4 flex flex-col space-y-16 overflow-y-scroll'>
        {editorState.map(({ type, id, props }) => {
            ...
            switch (type) {
                case "image":
                    return <DynamicImage key={id} alt={String(alt)} height={Number(height || 300)} width={Number(width || 500)} imageURL={String(imageURL || defaultImageURL)} />
                case "button":
                    return <Button height={Number(height)} width={Number(width)} text={String(text || "Default Text")} />
                default:
                    return null
            }
        })}
